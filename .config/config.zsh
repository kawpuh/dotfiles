...
commit() {
    ...
    message=$(git diff --no-ext-diff --no-color --staged | llm -s "Write a commit message. Respond with only the commit message. Keep it simple.")
    echo "$message"
...
status() {
    local explanation
    explanation=$(git diff --no-ext-diff --no-color "$@" | llm -s "Explain these git changes in a clear, concise way. Focus on what files were modified and how. Be brief.")
    git status
    echo "$explanation"
}
qcommit() {
